...
bool parentUsesSidePanelSurface(const QWidget* widget, const Holonight::ColorTokens& tok) {
  for (const QWidget* parent = widget != nullptr ? widget->parentWidget() : nullptr; parent != nullptr;
    ...
    if (parent->palette().color(QPalette::Window) == tok.surfaceElevated) {
      return true;
    }
  }
  return false;
...
qreal controlRadius(const QRectF& rect) {
  return Holonight::ShapeResolver::resolve(Holonight::SurfaceRole::Control, Holonight::CornerStyle::Rounded,
                                           rect.size())
      .radius;
}

bool isChamfered(Holonight::CornerMask corners, Holonight::Corner corner) { return corners.testFlag(corner); }

QPainterPath surfacePath(const QRectF& rect, const Holonight::ResolvedShape& shape) {
  QPainterPath path;
  if (rect.isEmpty()) {
    return path;
  }

  const auto cornerExtent = [&shape](Holonight::Corner corner) {
    return isChamfered(shape.chamfered_corners, corner) ? shape.chamfer : shape.radius;
  };
  const qreal top_left = cornerExtent(Holonight::Corner::TopLeft);
  const qreal top_right = cornerExtent(Holonight::Corner::TopRight);
  const qreal bottom_right = cornerExtent(Holonight::Corner::BottomRight);
  const qreal bottom_left = cornerExtent(Holonight::Corner::BottomLeft);

  path.moveTo(rect.left() + top_left, rect.top());
  path.lineTo(rect.right() - top_right, rect.top());
  if (isChamfered(shape.chamfered_corners, Holonight::Corner::TopRight)) {
    path.lineTo(rect.right(), rect.top() + top_right);
  } else {
    path.quadTo(rect.topRight(), QPointF{rect.right(), rect.top() + top_right});
  }
  path.lineTo(rect.right(), rect.bottom() - bottom_right);
  if (isChamfered(shape.chamfered_corners, Holonight::Corner::BottomRight)) {
    path.lineTo(rect.right() - bottom_right, rect.bottom());
  } else {
    path.quadTo(rect.bottomRight(), QPointF{rect.right() - bottom_right, rect.bottom()});
  }
  path.lineTo(rect.left() + bottom_left, rect.bottom());
  if (isChamfered(shape.chamfered_corners, Holonight::Corner::BottomLeft)) {
    path.lineTo(rect.left(), rect.bottom() - bottom_left);
  } else {
    path.quadTo(rect.bottomLeft(), QPointF{rect.left(), rect.bottom() - bottom_left});
  }
  path.lineTo(rect.left(), rect.top() + top_left);
  if (isChamfered(shape.chamfered_corners, Holonight::Corner::TopLeft)) {
    path.lineTo(rect.left() + top_left, rect.top());
  } else {
    path.quadTo(rect.topLeft(), QPointF{rect.left() + top_left, rect.top()});
  }
  path.closeSubpath();
  return path;
}

QPainterPath resolvedSurfacePath(const QRectF& rect, Holonight::SurfaceRole role,
                                 Holonight::CornerStyle style = Holonight::CornerStyle::Inherit) {
  return surfacePath(rect, Holonight::ShapeResolver::resolve(role, style, rect.size()));
}
...
void drawInputFrame(QPainter* painter, const QRect& rect, const QColor& color, qreal penWidth) {
  ...
  painter->setBrush(Qt::NoBrush);
  const QRectF frame_rect = strokedRectInside(rect, penWidth);
  painter->drawRoundedRect(frame_rect, controlRadius(frame_rect), controlRadius(frame_rect));
  painter->restore();
}
...
void drawInputPanel(QPainter* painter, const QRect& rect, const QColor& fill, const QColor& border, qreal penWidth) {
  ...
  const QRectF panel_rect = strokedRectInside(rect, penWidth);
...
void HoloniightStyle::polish(QWidget* widget) {
  ...
  if (surfaceRole == SurfaceRole::Window &&
      (palette.color(QPalette::Window) == tok.surfaceElevated || parentUsesSidePanelSurface(widget, tok))) {
  ...
  palette.setColor(QPalette::WindowText, tok.textPrimary);
  ...
  palette.setColor(QPalette::Text, tok.textPrimary);
  palette.setColor(QPalette::Button, tok.surfaceRaised);
  palette.setColor(QPalette::ButtonText, tok.textPrimary);
...
void HoloniightStyle::drawControl(ControlElement element, const QStyleOption* option, QPainter* painter,
  ...
  switch (element) {
    ...
    case CE_PushButtonLabel: {
      ...
      if (btnOpt != nullptr && (btnOpt->features & QStyleOptionButton::DefaultButton) != 0U) {
        const auto& tok = tokens();
        QStyleOptionButton labelOpt = *btnOpt;
        labelOpt.palette.setColor(QPalette::ButtonText, tok.onPrimary);
        QProxyStyle::drawControl(element, &labelOpt, painter, widget);
        return;
      }
      break;
    }
    ...
    case CE_ScrollBarSlider: {
      ...
      const QRectF indicator_rect = option->rect.adjusted(1, 1, -1, -1);
      painter->drawRoundedRect(indicator_rect, controlRadius(indicator_rect), controlRadius(indicator_rect));
      painter->restore();
      return;
    }
...
void HoloniightStyle::drawCheckBoxImpl(const QStyleOption* option, QPainter* painter, const QWidget* widget) const {
  ...
  if (checked) {
    ...
    painter->drawRoundedRect(borderRect, controlRadius(borderRect), controlRadius(borderRect));
...
void HoloniightStyle::drawHeaderImpl(const QStyleOption* option, QPainter* painter, const QWidget* widget) const {
  ...
  painter->setPen(Qt::NoPen);
  painter->setBrush(hovered ? tok.surfaceHover : tok.surface);
...
void HoloniightStyle::paintArrow(QPainter* painter, const QRect& rect, ArrowDirection direction, const QColor& color) {
  painter->save();
  painter->setRenderHint(QPainter::Antialiasing);
  const QPen pen{color, 1.5, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin};
  painter->setPen(pen);
  painter->setBrush(Qt::NoBrush);
  const qreal arrowX = rect.center().x();
  const qreal arrowY = rect.center().y();
  QPolygonF chevron;
  switch (direction) {
    ...
    case ArrowDirection::Left:
      chevron << QPointF{arrowX + 2, arrowY - 4} << QPointF{arrowX - 2, arrowY} << QPointF{arrowX + 2, arrowY + 4};
      break;
...
void HoloniightStyle::drawPanelButtonImpl(const QStyleOption* option, QPainter* painter) const {
  ...
  const auto* btnOpt = qstyleoption_cast<const QStyleOptionButton*>(option);
  const bool isDefault = btnOpt != nullptr && (btnOpt->features & QStyleOptionButton::DefaultButton) != 0U;

  QColor fill;
  if (isDefault) {
    if (pressed) {
      fill = tok.primaryPressed;
    } else if (hovered) {
      fill = tok.primaryHover;
    } else {
      fill = tok.primary;
    }
  } else {
    if (pressed) {
      ...
      fill = tok.surfaceHover;
    } else {
      fill = tok.surfaceRaised;
    }
  }
  const int borderWidth = 1;
  painter->setPen(QPen{focused ? tok.borderFocus : tok.borderPassive, static_cast<qreal>(borderWidth)});
  painter->setBrush(fill);
  const QRectF panel_rect = option->rect.adjusted(borderWidth, borderWidth, -borderWidth, -borderWidth);
  painter->drawRoundedRect(panel_rect, controlRadius(panel_rect), controlRadius(panel_rect));
  painter->restore();
}
...
void HoloniightStyle::drawPanelItemViewImpl(const QStyleOption* option, QPainter* painter) const {
  ...
  if (focused && !selected) {
    painter->setBrush(Qt::NoBrush);
    painter->setPen(QPen{tok.borderFocus, 1});
    painter->drawRoundedRect(option->rect.adjusted(1, 1, -1, -1), kRadiusTight, kRadiusTight);
  }
  painter->restore();
}
...
void HoloniightStyle::drawItemViewItemImpl(const QStyleOption* option, QPainter* painter, const QWidget* widget) const {
  ...
  if (selected) {
    painter->setBrush(tok.primary);
    const QRectF item_rect = opt->rect.adjusted(1, 1, -1, -1);
    painter->drawRoundedRect(item_rect, controlRadius(item_rect), controlRadius(item_rect));
  } else if (hovered) {
    painter->setBrush(tok.surfaceHover);
    const QRectF item_rect = opt->rect.adjusted(1, 1, -1, -1);
    painter->drawRoundedRect(item_rect, controlRadius(item_rect), controlRadius(item_rect));
  } else if ((opt->features & QStyleOptionViewItem::Alternate) != 0U) {
    painter->setBrush(tok.surfaceElevated);
    painter->drawRect(opt->rect);
  }
  ...
  QColor textColor = enabled ? tok.textPrimary : tok.textDisabled;
  if (selected) {
    textColor = tok.onPrimary;
  } else if (!enabled) {
    textColor = tok.textDisabled;
...
void HoloniightStyle::drawPrimitive(PrimitiveElement element, const QStyleOption* option, QPainter* painter,
  ...
  switch (element) {
    ...
    case PE_FrameDefaultButton: {
      painter->save();
      painter->setRenderHint(QPainter::Antialiasing);
      painter->setPen(QPen{tok.borderFocus, 2});
      painter->setBrush(Qt::NoBrush);
      const QRectF frame_rect = option->rect.adjusted(-1, -1, 1, 1);
      painter->drawRoundedRect(frame_rect, controlRadius(frame_rect), controlRadius(frame_rect));
      painter->restore();
      return;
    }
    ...
    case PE_FrameTabBarBase: {
      painter->save();
      painter->setPen(Qt::NoPen);
      painter->setBrush(tok.background);
      painter->drawRect(option->rect);
      painter->restore();
      return;
    }
    ...
    case PE_FrameMenu: {
      ...
      painter->setBrush(Qt::NoBrush);
      const QRectF frame_rect = strokedRectInside(option->rect, 1.0);
      painter->drawPath(resolvedSurfacePath(frame_rect, Holonight::SurfaceRole::Menu));
      painter->restore();
      return;
    }
    ...
    case PE_PanelMenu: {
      ...
      painter->drawPath(resolvedSurfacePath(QRectF{option->rect}, Holonight::SurfaceRole::Menu));
      painter->restore();
    ...
    case PE_PanelTipLabel: {
      ...
      painter->setPen(QPen{tok.borderPassive, 1.0});
      const QRectF panel_rect = strokedRectInside(option->rect, 1.0);
      painter->drawPath(resolvedSurfacePath(panel_rect, Holonight::SurfaceRole::Tooltip));
      painter->restore();
      return;
    }
    ...
    case PE_FrameFocusRect: {
      painter->save();
      painter->setPen(QPen{tok.borderFocus, 1, Qt::DotLine});
      painter->setBrush(Qt::NoBrush);
...
void HoloniightStyle::drawComplexControl(ComplexControl control, const QStyleOptionComplex* option, QPainter* painter,
  ...
  switch (control) {
    ...
    case CC_ComboBox: {
      ...
      const QRectF frame_rect = opt->rect.adjusted(borderWidth, borderWidth, -borderWidth, -borderWidth);
      painter->drawRoundedRect(frame_rect, controlRadius(frame_rect), controlRadius(frame_rect));
...
void HoloniightStyle::drawScrollBarImpl(const QStyleOptionComplex* option, QPainter* painter,
  ...
  painter->drawRoundedRect(handleRect, controlRadius(handleRect), controlRadius(handleRect));
  painter->restore();
}
...
void HoloniightStyle::drawSpinBoxImpl(const QStyleOption* option, QPainter* painter, const QWidget* widget) const {
  ...
  painter->setPen(QPen{border, static_cast<qreal>(borderWidth)});
  const QRectF frame_rect = opt->rect.adjusted(borderWidth, borderWidth, -borderWidth, -borderWidth);
  const qreal radius = controlRadius(frame_rect);
  painter->drawRoundedRect(frame_rect, radius, radius);

  QPainterPath clipPath;
  clipPath.addRoundedRect(frame_rect, radius, radius);
  ...
  const QColor idleBg = enabled ? tok.surface : tok.background;
  painter->setPen(Qt::NoPen);
...
void HoloniightStyle::drawToolButtonImpl(const QStyleOption* option, QPainter* painter, const QWidget* widget) const {
  ...
  painter->setPen(Qt::NoPen);
  painter->setBrush(bgColor);
  const QRectF panel_rect = opt->rect.adjusted(1, 1, -1, -1);
  painter->drawRoundedRect(panel_rect, controlRadius(panel_rect), controlRadius(panel_rect));

  if (focused && !idleFlatAutoRaise) {
    painter->setPen(QPen{tok.borderFocus, 1, Qt::DashLine});
    painter->setBrush(Qt::NoBrush);
    painter->drawRoundedRect(panel_rect, controlRadius(panel_rect), controlRadius(panel_rect));
  }
...
void HoloniightStyle::drawGroupBoxImpl(const QStyleOption* option, QPainter* painter, const QWidget* widget) const {
  ...
  painter->setPen(QPen{tok.borderPassive, 1});
  painter->setBrush(Qt::NoBrush);
  const QRectF control_rect = frameRect.adjusted(0, 0, -1, -1);
  painter->drawRoundedRect(control_rect, controlRadius(control_rect), controlRadius(control_rect));

  const QRect labelRect = subControlRect(CC_GroupBox, opt, SC_GroupBoxLabel, widget);
  painter->setPen(Qt::NoPen);
  painter->setBrush(tok.surface);
  painter->drawRect(labelRect.adjusted(-2, 0, 2, 0));
  painter->setPen(tok.textPrimary);
  painter->drawText(labelRect, Qt::AlignVCenter | Qt::AlignLeft, opt->text);
