...
Item {
    ...
    property font font: Qt.font({ family: HolonightTheme.uiFont, pointSize: HolonightTheme.bodySize })
    property color color: enabled ? HoloniightPalette.textSecondary : HoloniightPalette.textDisabled

    implicitWidth: keyGroups.length > 0 ? d.naturalWidth : literal.implicitWidth
    implicitHeight: keyGroups.length > 0 ? d.layout.height : literal.implicitHeight
    ...
    QtObject {
        ...
        function arrange() {
            const tokens = []
            const limit = root.wrap ? Math.max(1, root.width) : Infinity
